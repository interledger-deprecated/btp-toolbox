  ...
  describe('client reconnects', () => {
    beforeEach(function () {
      return this.server.start().then(() => {
        ...
        return this.client.send(this.packet, 'ws://localhost:8000/client/asdf')
      }).then(() => {
        return this.server.send(this.responsePacket, 'ws://localhost:8000/client/asdf')
      }).then(() => {
        return this.client.send(this.packet2, 'ws://localhost:8000/client/asdf')
      }).then(() => {
        this.server.send(this.packet3, 'ws://localhost:8000/client/asdf')
      }).then(() => {
        return this.client.stop()
      }).then(() => {
        const clientReconnected = new Promise(resolve => {
          this.onConnectedToServer.push((peerId) => {
            this.client.send(this.packet4, 'ws://localhost:8000/client/asdf').then(resolve)
          })
        })
