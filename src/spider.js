...
function BtpSpider (config, connectHandler, msgHandler) {
  this.peers = {}
  this.serversToClose = []
  this.config = config
      ...
      if (servers.length) {
        ...
        this.wss.on('connection', (ws, httpReq) => {
          ...
          if (this.peers[peerId]) {
            ...
            setTimeout(() => {
              for (let requestId in this.peers[peerId].unanswered) {
                this.send(this.peers[peerId].unanswered[requestId], peerId)
              }
            }, 100)
          } else {
            this.peers[peerId] = {
        ...
        return new Promise((resolve, reject) => {
          this.peers[peerId].ws.send(msg, {}, (err) => {
            if (err) {
              ...
              if (BtpPacket.isRequest(obj.type)) {
                this.peers[peerId].unanswered[obj.requestId] = obj
              }
              resolve()
            }
          })
